...
function toggleEdit(sectionId, event) {
    ...
    formElements.forEach(element => {
        if (!element.classList.contains('edit-btn') && !element.classList.contains('save-btn')) {
            ...
            // Enable rich text toolbars when enabling rich text editors
...
function saveSection(sectionId, event) {
    ...
    formElements.forEach(element => {
        if (!element.classList.contains('edit-btn') && !element.classList.contains('save-btn')) {
            ...
            // Disable rich text toolbars when disabling rich text editors
            if (element.classList.contains('rich-text-editor')) {
                const toolbar = element.previousElementSibling;
                if (toolbar && toolbar.classList.contains('rich-text-toolbar')) {
                    const toolbarButtons = toolbar.querySelectorAll('button');
...
function previewReport() {
    ...
    const auditName = auditNameInput ? 
        (auditNameInput.classList.contains('rich-text-editor') ? 
            auditNameInput.innerHTML || '[AUDIT NAME]' : 
            auditNameInput.value || '[AUDIT NAME]') : 
        '[AUDIT NAME]';
    ...
    const executiveSummary = executiveSummaryInput ? 
        (executiveSummaryInput.classList.contains('rich-text-editor') ? 
            executiveSummaryInput.innerHTML || 'No executive summary provided.' : 
            executiveSummaryInput.value || 'No executive summary provided.') : 
        'No executive summary provided.';
    
    const background = backgroundInput ? 
        (backgroundInput.classList.contains('rich-text-editor') ? 
            backgroundInput.innerHTML || 'No background information provided.' : 
            backgroundInput.value || 'No background information provided.') : 
        'No background information provided.';
    
    const scopeIncluded = scopeIncludedInput ? 
        (scopeIncludedInput.classList.contains('rich-text-editor') ? 
            scopeIncludedInput.innerHTML || 'No scope inclusions specified.' : 
            scopeIncludedInput.value || 'No scope inclusions specified.') : 
        'No scope inclusions specified.';
    
    const scopeExcluded = scopeExcludedInput ? 
        (scopeExcludedInput.classList.contains('rich-text-editor') ? 
            scopeExcludedInput.innerHTML || 'No scope exclusions specified.' : 
            scopeExcludedInput.value || 'No scope exclusions specified.') : 
        'No scope exclusions specified.';
    ...
    findings.forEach((finding, findingIndex) => {
        ...
        // Handle rich text editor content (HTML) or regular textarea
        const description = descriptionEditor ? 
            (descriptionEditor.classList.contains('rich-text-editor') ? 
                (descriptionEditor.innerHTML || 'No description provided') : 
                (descriptionEditor.value.replace(/\n/g, '<br>') || 'No description provided')) : 
        ...
        actionItems.forEach((actionItem, actionIndex) => {
            ...
            const actionDescInput = actionItem.querySelector('[name*="actionItem"]');
            ...
            const actionDescription = actionDescInput ? 
                (actionDescInput.classList.contains('rich-text-editor') ? 
                    actionDescInput.innerHTML.trim() || '' : 
                    actionDescInput.value.trim() || '') : 
                '';
...
async function saveAllFormData() {
    try {
        ...
        const auditName = auditNameElement ? 
            (auditNameElement.classList.contains('rich-text-editor') ? 
                auditNameElement.innerHTML || '' : 
                auditNameElement.value || '') : 
            '';
        
        const executiveSummary = executiveSummaryElement ? 
            (executiveSummaryElement.classList.contains('rich-text-editor') ? 
                executiveSummaryElement.innerHTML || '' : 
                executiveSummaryElement.value || '') : 
            '';
        
        const background = backgroundElement ? 
            (backgroundElement.classList.contains('rich-text-editor') ? 
                backgroundElement.innerHTML || '' : 
                backgroundElement.value || '') : 
            '';
        
        const scopeIncluded = scopeIncludedElement ? 
            (scopeIncludedElement.classList.contains('rich-text-editor') ? 
                scopeIncludedElement.innerHTML || '' : 
                scopeIncludedElement.value || '') : 
            '';
        
        const scopeExcluded = scopeExcludedElement ? 
            (scopeExcludedElement.classList.contains('rich-text-editor') ? 
                scopeExcludedElement.innerHTML || '' : 
                scopeExcludedElement.value || '') : 
            '';
        ...
        document.querySelectorAll('.finding-container').forEach((findingElem, findingIndex) => {
            ...
            const descriptionEditor = findingElem.querySelector('[name*="findingDescription"]');
            ...
            findingElem.querySelectorAll('.action-item-container').forEach((actionItem, actionIndex) => {
                ...
                actionItems.push({
                    actionItem: `${actionName}${actionDescription ? ': ' + actionDescription : ''}`,
                    dueDate,
                    owner
                });
            });
...
// Handle input events to update toolbar
